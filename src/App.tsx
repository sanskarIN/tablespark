...
export default function App() {
  ...
  useEffect(() => {
    ...
    const applyTheme = () => {
      root.dataset.theme = state.settings.theme === 'system' ? resolveSystemTheme() : state.settings.theme;
    };
    applyTheme();
  ...
  return (
    <div className="app-shell">
      ...
      <header className="topbar no-print">
        <button className="brand" type="button" onClick={() => setView('tables')} aria-label="Go to multiplication tables">
          <img src="/logo.svg" alt="" width="42" height="42" />
          <span><strong>{copy.appName}</strong><small>{copy.tagline}</small></span>
        </button>
        <div className="profile-chip" title="Active offline profile">{activeProfile.name}</div>
      </header>

      <nav className="navigation no-print" aria-label="Primary navigation">
        {views.map((item, index) => (
          <button key={item} type="button" className={view === item ? 'nav-button active' : 'nav-button'} onClick={() => setView(item)} aria-current={view === item ? 'page' : undefined} title={`Alt+${index + 1}`}>
      ...
      <footer className="footer no-print">
        ...
        <a href="https://buymeacoffee.com/sanskarIN" target="_blank" rel="noreferrer">Support TableSpark</a>
      </footer>
    </div>
  );
}
