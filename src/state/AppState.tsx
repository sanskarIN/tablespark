...
import { importState as parseImportedState, loadState, saveState } from '../infrastructure/storage';
...
function makeDefaultState(): PersistedState {
  ...
  return { schemaVersion: 1, activeProfileId: profile.id, profiles: [profile], settings: defaultSettings };
}
...
export function AppStateProvider({ children }: { readonly children: ReactNode }) {
  ...
  const value = useMemo<AppStateValue>(() => ({
    ...
    activeProfile,
    setActiveProfile: (id) => {
      if (state.profiles.some((profile) => profile.id === id)) setState((current) => ({ ...current, activeProfileId: id }));
    },
    addProfile: (name) => {
    ...
    deleteProfile: (id) => {
      setState((current) => {
        ...
        const activeProfileId = current.activeProfileId === id ? (profiles[0]?.id ?? current.activeProfileId) : current.activeProfileId;
    ...
    recordAttempt: (attempt) => setState((current) => ({
      ...current,
      profiles: current.profiles.map((profile) => profile.id === current.activeProfileId ? applyAttempt(profile, attempt) : profile),
    })),
    replaceFromBackup: (raw) => setState(parseImportedState(raw)),
    resetProgress: () => setState((current) => ({
      ...current,
      profiles: current.profiles.map((profile) => profile.id === current.activeProfileId ? { ...profile, mastery: {}, mistakes: [] } : profile),
    })),
  }), [activeProfile, state]);
