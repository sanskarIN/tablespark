...
- committed npm and Cargo dependency lockfiles
...
Vite serves development on `http://localhost:5173` by default. `npm ci` installs the dependency graph committed in `package-lock.json` and intentionally fails if the manifest and lockfile drift.
...
`npm run check` includes the Node-based native configuration tests/check, so package version, pinned npm toolchain, Tauri configuration, identifier, locked Cargo verification, mobile minimums, and required scripts cannot silently drift. Rust/native compilation is separately exercised by the Native Cross-Platform GitHub Actions workflow.
...
Generated native projects under `src-tauri/gen/` and Rust output under `src-tauri/target/` are intentionally ignored and regenerated by the Tauri toolchain. `package-lock.json` and `src-tauri/Cargo.lock` are intentionally tracked reproducibility metadata.
...
- locked npm/Cargo dependency resolution for reproducible verification;
...
`.github/workflows/native.yml` compiles the desktop native application without installer signing/bundling on Windows, macOS, and Linux, compiles an Android debug APK, and compiles an iOS simulator application on appropriate GitHub-hosted operating systems.
