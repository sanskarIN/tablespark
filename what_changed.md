...
# 2026-08-23 deterministic release-gate continuation

## 48. Exact failing head was inspected before changing code

The continuation started from exact candidate `141e1a87e48348640cff29cab8aad0f70fbb69a3` rather than assuming the previous source review was green.

Observed workflow evidence for that SHA:

- Native Cross-Platform run `32353757342` succeeded across its maintained compile matrix.
- CodeQL run `32353757407` succeeded.
- CI run `32353757367` failed.
  - `format:check` reported 25 maintained files that still needed the repository-pinned Prettier result at that newer head.
  - browser E2E exposed strict-selector/version/keyboard assertions that did not accurately model the rendered browser behavior.
- Release Visual Evidence run `32353757320` failed because all four screenshot cases used an ambiguous `Tables` button locator, so no screenshot artifact was produced.

These failures were treated as evidence and fixed; they were not relabeled as passes.

## 49. Browser regression and E2E reliability fixes

Focused fixes in this continuation:

- `src/App.tsx` accepts both browser representations of the help shortcut: `event.key === '?'` and `Shift+/` reported as `/` plus `shiftKey`.
- `src/App.test.tsx` adds a shifted-slash regression test.
- `e2e/localization.spec.ts` verifies visible About version `2.0.12` semantically inside the main content rather than requiring one standalone text node that the markup intentionally splits.
- `e2e/localized-errors.spec.ts` filters Hindi status assertions by their expected error text so the concurrent offline-ready live region cannot make the locator ambiguous.
- `e2e/smoke.spec.ts` selects the exact `Practice` navigation button instead of also matching the worksheet action `Print practice worksheet`.
- `e2e/release-evidence.spec.ts` selects the exact `Tables` navigation button instead of also matching the brand control `Go to multiplication tables`.

The changes address real browser-test ambiguity without weakening application behavior or deleting coverage.

## 50. Repository formatter result is now committed

The repository-pinned Prettier 3.6.2 formatter was executed on a GitHub-hosted Node 22.12.0 environment and its deterministic diff was committed across the files reported by CI.

The earlier section 44 is historical evidence of the unresolved state at the previous handoff; it is now superseded by this section. `format:check` remains enabled and was not relaxed.

## 51. Real npm and Cargo lockfiles are now committed

Permanent reproducibility files now tracked:
...
They were generated by the real npm/Cargo toolchains from the maintained dependency manifests; they were not fabricated manually.

`package.json` now additionally declares:

```text
packageManager: npm@10.9.0
```

The earlier section 45 is historical evidence of the prior pending state and is superseded by this section.

## 52. Deterministic JavaScript installation is enforced across automation

All maintained automated JavaScript installation paths now use:

```bash
npm ci --no-fund --no-audit
```

Updated permanent workflows:
...
The explicit production dependency advisory audit remains a separate release/quality action rather than being hidden inside installation.

CodeQL no longer relies on an opaque autobuild path: it explicitly selects Node 22.12.0, installs the committed lockfile graph, builds the application, and then analyzes it.

## 53. Rust dependency drift now fails verification

`native:check` now runs:

```bash
npm run native:prepare && cargo check --locked --manifest-path src-tauri/Cargo.toml
```

The native configuration validator also requires:

- `packageManager` to remain `npm@10.9.0`;
- `native:check` to retain `cargo check --locked`.

Its test fixture now contains these invariants and includes a regression that deliberately drifts both values to prove the validator rejects the change.

## 54. Temporary write-enabled helpers were removed

A branch-only one-shot canonicalization workflow was used to let the pinned GitHub runner generate the exact Prettier output and both real lockfiles. It was deleted immediately after those outputs landed.

A later documentation helper was created while exploring a mechanical inventory update but did not produce branch commits during the active session. It was explicitly deleted and the permanent documentation changes were committed directly instead.

The maintained workflow directory again contains only the five intended product/release workflows. No temporary helper with `contents: write` is intended to remain in the final tracked tree.

## 55. Exhaustive repository inventory is corrected to 174 files

The older 171-file reference was already known to be stale because `tsconfig.e2e.json` had been added after that checkpoint. The two new real dependency lockfiles add two more permanent tracked files.

`docs/repository-file-reference.md` now lists **174 tracked files** individually:

- the prior 171-file published reference;
- previously omitted `tsconfig.e2e.json`;
- new `package-lock.json`;
- new `src-tauri/Cargo.lock`.

The reference now also distinguishes `tsconfig.node.json` from `tsconfig.e2e.json`, documents locked npm/Cargo synchronization, and explicitly states that the two lockfiles are source-controlled dependency metadata rather than generated cleanup output.

The earlier section 46 is historical evidence of the known drift and is superseded by this corrected inventory.

## 56. Setup and command documentation now matches the executable toolchain

`docs/setup.md` now:

- requires npm 10.9.0 for the repository-pinned npm toolchain;
- uses `npm ci` for normal JavaScript setup;
- documents `package-lock.json` and `src-tauri/Cargo.lock`;
- explains locked Cargo verification;
- includes npm and Cargo lock-drift troubleshooting.

`docs/commands-reference.md` now:

- identifies npm 10.9.0 as the pinned repository toolchain;
- uses `npm ci` / `npm ci --no-fund --no-audit` rather than mutable installation guidance;
- documents `cargo check --locked` exactly;
- documents the toolchain invariants enforced by the native configuration checker;
- distinguishes committed lockfiles from generated output.

## 57. Atomic commit sequence in this continuation

The continuation intentionally used many focused commits rather than one broad rewrite. Permanent changes include separate commits for:

- keyboard shortcut normalization;
- shifted-slash regression coverage;
- version visibility E2E semantics;
- localized status locator disambiguation;
- practice navigation locator disambiguation;
- release-evidence navigation locator disambiguation;
- repository-wide canonical Prettier formatting;
- JavaScript dependency lockfile generation;
- Rust dependency lockfile generation;
- shared CI locked installation;
- visual-evidence locked installation;
- native CI locked installation;
- release workflow locked installation;
- deterministic CodeQL build;
- pinned npm package-manager metadata;
- locked Cargo verification;
- native toolchain invariant validation;
- native toolchain invariant regression tests;
- locked setup documentation;
- locked command documentation;
- exhaustive 174-file inventory correction;
- removal of both temporary maintenance helpers.

No empty/fake commits were added merely to inflate the count.

## 58. Exact-head verification remains authoritative

This `what_changed.md` update itself creates another candidate SHA, so earlier successful runs cannot be used as final exact-head evidence for the new head.

After this commit, continue by inspecting PR #4's new exact head and only that head's:
...
If a latest-head job fails, inspect its exact log and fix the actual failure. If all automated gates pass, record the immutable candidate SHA and exact run IDs/results in PR metadata rather than hard-coding the SHA into this tracked handoff and creating another candidate.

PR #4 must remain draft/unmerged and `v2.0.12` must remain uncreated until the applicable manual accessibility, Hindi, production-origin/PWA, installed-native, physical-device, signing/notarization/store, and downloaded-artifact checks are actually completed.
